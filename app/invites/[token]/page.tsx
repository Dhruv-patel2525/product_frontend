...
export default function InvitePage() {
  ...
  const { data: invitation, isLoading, error } = useGetInvitation(token, !!token);
  const acceptInvitation = useAcceptInvitation(orgId || '');

  // Set orgId when invitation loads
  useEffect(() => {
    if (invitation) {
      setOrgId(invitation.org_id.toString());
    }
  }, [invitation]);

  const handleAccept = async () => {
    if (!invitation || !isLoggedIn) return;
        ...
        {invitation.status === 'ACCEPTED' ? (
          ...
          <div>
            {!isLoggedIn ? (
              <div>
                <p className="text-gray-600 mb-4">
                  Please log in to accept this invitation. Use the email address that received the invitation.
                </p>
